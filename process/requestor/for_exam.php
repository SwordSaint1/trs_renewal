...
if ($method == 'fetch_for_exam') {
	...
	$query = "SELECT trs_renewal_sched.id,trs_renewal_sched.training_code,trs_renewal_sched.shift,trs_renewal_sched.start_date,trs_renewal_sched.end_date,
			...
			LEFT JOIN trs_renewal_request ON trs_renewal_request.tr_code = trs_renewal_sched.training_code WHERE trs_renewal_request.tr_code != '' AND trs_renewal_sched.sched_stat = 1 AND trs_renewal_sched.start_date LIKE '$start%' AND trs_renewal_sched.shift LIKE '$shift%' GROUP BY trs_renewal_sched.training_code";
