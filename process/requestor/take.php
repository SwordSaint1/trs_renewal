...
WHERE trs_renewal_sched.start_date LIKE '$start%' AND trs_renewal_sched.shift LIKE '$shift%' AND trs_renewal_request.exam_status = 'Failed' AND trs_renewal_request.ft_status = 0 AND trs_renewal_sched.sched_stat = 0 GROUP BY trs_renewal_sched.training_code";
...
if ($method == 'fetch_prev_take_req') {
	...
	$c = 0;
	$query = "SELECT * FROM trs_renewal_request WHERE tr_code = '$tr_code' AND final_status != '' AND ft_status = 0 AND exam_status = 'Failed'";
	$stmt = $conn->prepare($query);
	$stmt->execute();
	if ($stmt->rowCount() > 0) {
		foreach($stmt->fetchALL() as $j){
